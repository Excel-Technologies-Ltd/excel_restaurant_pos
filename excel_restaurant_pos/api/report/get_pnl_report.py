    ...
    try:
        ...
        if pnl_section in ("income", "both"):
            income_data = _build_income_data(
                start_date, end_date, type, sub_type, company
            )
        ...
        frappe.throw(
            frappe._(
                "Failed to fetch PnL Report. Please contact your system administrator."
            ),
            frappe.ValidationError,
        )
...
def _build_income_data(start_date, end_date, type_filter, sub_type_filter, company):
    ...
    entries = _fetch_income_entries(
        start_date, end_date, type_filter, sub_type_filter, company
    )
...
def _build_expense_data(start_date, end_date, type_filter, sub_type_filter, company):
    ...
    entries = _fetch_expense_entries(
        start_date, end_date, type_filter, sub_type_filter, company
    )
    total = flt(sum(r.amount for r in entries), 2)

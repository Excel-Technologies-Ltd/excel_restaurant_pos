...
def send_scheduled_order_notification_to_staff(invoice):
    ...
    print(
        f"[ScheduledNotif] Starting notifications for invoice: {doc.name} | Title: {title}"
    )
    for role in target_roles:
        ...
        try:
            ...
            print(
                f"[ScheduledNotif] Found {len(user_emails)} user(s) for role '{role}': {user_emails}"
            )
            frappe.logger().info(
            ...
            if settings.scheduled_order_reminder_template:
                print(
                    f"[ScheduledNotif] Using email template: {settings.scheduled_order_reminder_template}"
                )
                try:
                    ...
                    for user_email in user_emails:
                        try:
                            ...
                            print(
                                f"[ScheduledNotif] Sending EMAIL to: {user_email} | Subject: {subject}"
                            )
                            # Send email
                            frappe.sendmail(
                            ...
                            print(
                                f"[ScheduledNotif] EMAIL sent successfully to: {user_email}"
                            )
                            frappe.logger().info(
                ...
                print(
                    f"[ScheduledNotif] WARNING: No email template configured in ArcPOS Settings — skipping email"
                )
                frappe.logger().warning(
...
def auto_cancel_rejected_invoices():
    ...
    for invoice_name in invoices:
        try:
            ...
            cancelled_count += 1
            frappe.logger().info(
                f"Auto-cancelled rejected Sales Invoice: {invoice_name}"
            )
        except Exception as e:
            frappe.db.rollback()
            frappe.log_error(
